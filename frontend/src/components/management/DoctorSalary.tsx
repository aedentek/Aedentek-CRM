...
const DoctorSalary: React.FC = () => {
  ...
  const getDoctorPhotoUrl = (photoPath: string) => {
    ...
    if (photoPath.startsWith('Photos/') || photoPath.startsWith('Photos\\')) {
      return `${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}/${photoPath.replace(/\\/g, '/')}`;
    }
  ...
  const handleSaveMonthlyRecords = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/doctor-salaries/save-monthly-records`, {
        method: 'POST',
        headers: {
  ...
  const fetchMonthlyAdvanceAmount = async (doctorId: string, month: number, year: number) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/doctor-advance/monthly-total?doctorId=${doctorId}&month=${month}&year=${year}`);
      const data = await response.json();
  ...
  const fetchPaymentHistory = async (doctorId: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/doctor-salaries/${doctorId}/history`);
      const data = await response.json();
      if (data.success) {
  ...
  const confirmDeletePayment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/doctor-salaries/payment/${paymentToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const checkCarryForward = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/doctor-salaries/carry-forward/${month}/${year}`);
      const data = await response.json();
