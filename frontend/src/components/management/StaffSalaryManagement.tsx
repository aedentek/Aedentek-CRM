...
const SalaryPayment: React.FC = () => {
  ...
  const getStaffImageUrl = (photo: string | undefined | null): string => {
    ...
    if (photo.startsWith('/') || photo.startsWith('http')) {
      return `${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${photo.startsWith('/') ? '' : '/'}${photo}`;
    }
  ...
  const handleSaveMonthlyRecords = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/staff-salaries/save-monthly-records`, {
        method: 'POST',
        headers: {
