...
const PatientMedicalRecord: React.FC = () => {
  ...
  const handleAddRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/patient-medical-records/json`, {
        method: 'POST',
        headers: {
          ...
          >
            ...
            <div className="p-4 flex flex-col items-center">
              <div className="relative max-w-full max-h-[60vh] overflow-hidden rounded-lg">
                <img
                  src={`${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${viewImageModal.images[viewImageModal.currentIndex]}`}
              ...
              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => {
                    const link = document.createElement('a');
                    link.href = `${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${viewImageModal.images[viewImageModal.currentIndex]}`;
                ...
                <button
                  onClick={() => {
                    window.open(`${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${viewImageModal.images[viewImageModal.currentIndex]}`, '_blank');
                  }}
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      if (isImage) {
                                        ...
                                        const fileUrl = `${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${record.images[0]}`;
                                  ...
                                  <button
                                    onClick={async () => {
                                      try {
                                        ...
                                        const fileUrl = `${import.meta.env.VITE_API_URL?.replace(/\/api$/, '')}${imagePath}`;
